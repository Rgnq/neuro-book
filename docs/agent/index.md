...
| Subject RAG | `simulator.actor` 的 subject 级长期记忆召回，只检索当前 subject 的 `events.jsonl` 和 `memory.jsonl`。 |
...
- [Subject RAG 记忆](./subject-rag-memory.md)：`simulator.actor` 如何用 `events.jsonl` / `memory.jsonl` 减少长线失忆。
